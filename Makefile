...
fix: python
	@pyenv exec tox -qe lint -- ruff check --fix-only test_pyapp tests bin

.PHONY: noqa
$(call help,make noqa,"add noqa comments to suppress lint violations")
noqa: python
	@pyenv exec tox -qe lint -- ruff check --add-noqa test_pyapp tests bin
...
docker:
	@git archive --format=tar HEAD | docker build -t hypothesis/test-pyapp:dev -
